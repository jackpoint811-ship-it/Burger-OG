...
- **Refinamiento UI/UX Integral & Accesibilidad WCAG 2.1 AA en Public Order V3 (2026-08-26)**: Estandarización de terminología canónica a nivel global (sustituido "SPEI" por "Transferencia" en `OrderSuccessModal.tsx` y `CheckoutDrawer.tsx`), corrección de tokens visuales de hover/fondos, feedback táctil con `whileTap` en `ProductCard.tsx`, accesibilidad ARIA en acordeones de combos en `ProductDetailDrawer.tsx` (`aria-expanded`, `aria-controls`) y optimización de targets táctiles $\ge 38\text{px}$ en `FeaturedRail.tsx`.
