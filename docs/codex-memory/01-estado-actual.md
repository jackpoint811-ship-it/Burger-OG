...
- **Refinamiento UI/UX Integral & Accesibilidad WCAG 2.1 AA en Public Order V3 (2026-08-26)**: Estandarización de terminología canónica a nivel global (sustituido "SPEI" por "Transferencia" en `OrderSuccessModal.tsx` y `CheckoutDrawer.tsx`), alineación de tokens oficiales Tailwind v4 con ratios de contraste AAA (`#0F172A` / `#475569` en Light y `#FFFFFF` / `#CBD5E1` en Dark), feedback táctil con `whileTap` en `ProductCard.tsx`, navegación accesible por teclado en `BannerCarousel.tsx`, accesibilidad ARIA en acordeones de combos en `ProductDetailDrawer.tsx` (`aria-expanded`, `aria-controls`), vinculación explícita `htmlFor`/`id`/`aria-describedby` en inputs de `CheckoutDrawer.tsx` y optimización de targets táctiles en `FeaturedRail.tsx` y `ToastContainer.tsx` (PR #586).
