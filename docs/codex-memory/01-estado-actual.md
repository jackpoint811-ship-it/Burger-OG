...
- **Header Simplificado V3 & Semáforo de Torres por Color (2026-08-26)**: Reubicación del estado operativo global a la cabecera principal junto al nombre de la marca con formato directo (`🟢 Abierto` / `🔴 Cerrado`), eliminación total del botón redundante "Horarios" y simplificación radical de la sección de torres a una franja ultra compacta sin scroll horizontal (`overflow-x-auto` eliminado) con píldoras semánticas por color (`🟢 Verde = Abierto/Hoy`, `🟡 Amarillo = Programar/Próxima ruta`, `🔴 Rojo = Pausado/Cerrado`) que abren interactivamente el modal de horarios de entrega al hacer clic.
