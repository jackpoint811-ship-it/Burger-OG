...
### 9. Limpieza de seguridad y arquitectura (PR cleanup/v3-audit-hardcoded-fallbacks)

**Commits 688630f → ce3e017** (branch `cleanup/v3-audit-hardcoded-fallbacks`):

| Commit | Fix | Qué | Por qué |
|--------|-----|-----|---------|
| `688630f` | #481 + #482 | `bank-payment-config` + `mock-data` aislados del barrel público `@config/index` | Datos bancarios reales (Yolitzin Ameyali) y mocks nunca deben ser importables por la app pública; ahora solo `@config/bank-payment-config` (interno) y `@config/mock-data` (interno) |
| `8d8ec91` | #483 | `FALLBACK_TOWERS` → error 503/500 en `tower-schedules.ts` | Servidor ya no devuelve torres GGA/Valcob hardcodeadas con 200 OK; error explícito cuando D1 no disponible |
| `13cd233` | #484 | Logging en `optionalAll`/`optionalFirst` (`menu-v2.ts`) | D1 fallaba en silencio; ahora registra `[menu-v2] optionalAll failed (menu_items):` etc. para diagnóstico |
| `75086d5` | #485 | `resolveCatalogAssetUrl` centralizado en `packages/config/src/assets.ts` | 3 implementaciones locales (`catalog-mode.ts`, `PublicOrderApp.tsx`, `RafflesAdminPanel.tsx`) + `StoreBannersTool.getAssetUrl()` eliminadas → 1 función canónica exportada desde `@config/index` |
| `1e1debd` | #486 | SSE `live-stream` filtrado por `environment` (`orders-v2-admin/live-stream.ts`) | JOIN con `orders_v2.source` para que preview no reciba eventos de producción y viceversa |
| `ce3e017` | #488 | `TERMINAL_STATUSES` centralizado en `_orders-v2-utils.ts` | 4 duplicados (`_orders-v2-utils.ts`, `orders-v2-admin.ts`, `InternalChekeoApp.tsx`, `KitchenQueue.tsx`) → 1 exportado |

**Archivos modificados (14):**
- `packages/config/src/assets.ts` (nuevo), `packages/config/src/index.ts`
- `apps/public-order-v2/src/lib/catalog-mode.ts`, `menu-v2.ts`, `PublicOrderApp.tsx`
- `apps/internal-chekeo-v2/src/components/InternalChekeoApp.tsx`, `RafflesAdminPanel.tsx`, `kitchen/KitchenQueue.tsx`, `kitchen/kitchen-helpers.ts`, `v3/StoreBannersTool.tsx`
- `functions/api/_orders-v2-utils.ts`, `menu-v2.ts`, `orders-v2-admin/live-stream.ts`, `tower-schedules.ts`

**Validaciones:**
- ✅ `npm run typecheck` (0 errores)
- ✅ `npm run build:public` (414-415 módulos)
- ✅ `npm run build:internal` (1979 módulos, warning chunk 678 kB pre-existente)
- ✅ `git diff --check` limpio

**No incluido (aplazado a backlog V3 por decisión):**
- #487: `audit_logs` en `cash-cut.ts` — tabla `orders_v2_audit_logs` tiene `order_id NOT NULL` FK; requiere migración de schema.

### 10. Contaminación residual y siguientes pasos
...
| `RafflesAdminPanel` | `resolveAssetUrl` local | `@config/index` canónico |
| `StoreBannersTool` | `getAssetUrl` sin validación | `resolveCatalogAssetUrl` seguro |
...
5. **Barrel público `@config/index` solo expone contratos/runtime-env** —
   `bank-payment-config` y `mock-data` movidos a imports directos internos.
6. **Fallbacks de servidor devuelven errores explícitos (500/503)**, no 200 con mock.
7. **Logging obligatorio** en helpers tolerantes (`optionalAll`/`optionalFirst`).
8. **Assets resueltos vía `@config/index` canónico** — elimina 3+ copias.

## Pendiente (backlog V3)

- `PR5 → Drawers/UX`: polish de micro-interacciones y responsive fino.
- Separar los archivos NO-V3 del PR #480 en PRs independientes.
- Auditoría visual live (browser) de los 6 componentes migrados.
- Validación de dark mode (`theme-dark`) en componentes V3.
- #487: `audit_logs` en `cash-cut.ts` (requiere migración schema `order_id` nullable).
- Auditoría visual de los 6 componentes V3 migrados en PR #480.
