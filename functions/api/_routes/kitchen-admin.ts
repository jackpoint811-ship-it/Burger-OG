...
kitchenAdminRouter.get('/summary-k', async (c) => {
  ...
  try {
    const recipeCountRow = await c.env.BOG_MENU_DB.prepare('SELECT COUNT(*) AS count FROM product_ingredient_recipes_v2').first<{ count: number }>();
    ...
    const itemsResult = await c.env.BOG_MENU_DB.prepare(itemsSql).bind(fromUtc, toUtc, environmentCondition.binding).all<ItemRow>();
    ...
    const recipeRows = hasRecipes && consumedQtyBySku.size
      ? (
          ...
          ORDER BY ing.sort_order ASC, ing.name ASC`
            ...
            .bind(...consumedQtyBySku.keys())
            .all<IngredientRecipeRow>()
        ).results ?? []
      : [];
    ...
    const estimatedCosts = ingredients
      .map((ingredient) => ingredient.estimatedCostCents)
