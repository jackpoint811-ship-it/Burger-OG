...
import type { RaffleActiveResponse, RaffleCampaignV2, RaffleTicketsLookupResponse, RaffleTicketsLookupResult } from '../../../packages/config/src';
import { errorResponse, json, normalizePhone } from '../_orders-v2-utils';

export const rafflesRouter = new Hono<AppEnv>();
...
const safeActivePayload = (campaign: RaffleCampaignV2 | null): RaffleActiveResponse => ({
  ok: true,
  data: {
    campaign: campaign
      ? {
...
const loadActiveCampaign = async (db: D1Database) =>
  db
    .prepare(
...
const loadReferralCodeByCode = async (db: D1Database, campaignId: string, code: string) =>
  db
    .prepare(
    ...
    .first<ReferralCodeLookupRow>();

const loadReferralCodeByPhone = async (db: D1Database, campaignId: string, phone: string) =>
  db
    .prepare(
      `SELECT code, owner_name, owner_phone, is_active
       FROM raffle_referral_codes_v2
...
const mapReferralCode = (row: ReferralCodeLookupRow | null | undefined): RaffleTicketsLookupResult['referralCode'] =>
  row
    ? {
        code: row.code,
...
const loadParticipant = async (db: D1Database, campaign: RaffleCampaignRow, phone: string): Promise<RaffleTicketsLookupResult['participant']> => {
  ...
  if (startsAt) {
    ...
    orderBindings.push(startsAt);
  }
  if (endsAt) {
    orderConditions.push('o.created_at <= ?');
    orderBindings.push(endsAt);
  }
  ...
  for (const row of orderRows.results ?? []) {
    ...
    const burgerTickets = itemKind === 'burger' || itemKind === 'combo' ? Math.max(0, Number(row.qty) || 0) * ticketPerBurger : 0;
  ...
  const referralRows = await db
    ...
    .bind(campaign.id, phone)
    .all<ReferralTicketRow>();
  ...
  const manualExtraTicketsRow = await db
    .prepare(
    ...
    .bind(campaign.id, participantKey)
    .first<{ manual_extra_tickets: number }>();
  ...
  if (!participant && manualExtraTickets > 0) {
    const latestAdjustmentRow = await db
      .prepare(
      ...
      .bind(campaign.id, participantKey)
      .first<{ participant_name: string; participant_phone_masked: string; created_at: string; updated_at: string }>();
...
// GET /api/raffles-v2/active
rafflesRouter.get('/active', async (c) => {
  if (!c.env.BOG_MENU_DB) return json(200, safeActivePayload(null));
  ...
  try {
    ...
    return json(200, safeActivePayload(row ? mapCampaign(row) : null));
  } catch {
    return json(200, safeActivePayload(null));
  }
});

// GET /api/raffles-v2/lookup
rafflesRouter.get('/lookup', async (c) => {
  if (!c.env.BOG_MENU_DB) return json(200, emptyResult(null));

  const phone = normalizePhone(c.req.query('phone'));
  const code = normalizeLookupCode(c.req.query('code'));
  ...
  try {
    const campaign = await loadActiveCampaign(c.env.BOG_MENU_DB);
    if (!campaign) return json(200, emptyResult(null));
    const publicCampaign = mapPublicCampaign(campaign);

    const rawCodeRow = code ? await loadReferralCodeByCode(c.env.BOG_MENU_DB, campaign.id, code) : null;
    ...
    const participant = lookupPhone ? await loadParticipant(c.env.BOG_MENU_DB, campaign, lookupPhone) : null;
    const phoneCodeRow = !codeRow && lookupPhone ? await loadReferralCodeByPhone(c.env.BOG_MENU_DB, campaign.id, lookupPhone) : null;
