...
export const referralTicketsRouter = new Hono<AppEnv>();
...
referralTicketsRouter.get('/', async (c) => {
  if (!c.env.BOG_MENU_DB) return json(503, { ok: false, error: { code: 'MISSING_DB', message: 'Base de datos no configurada.' } });

  const phone = onlyDigits(c.req.query('phone'));
  if (phone.length < 8) return json(400, { ok: false, error: { code: 'INVALID_PHONE', message: 'Telefono invalido.' } });

  try {
    const campaign = await c.env.BOG_MENU_DB.prepare(
    ...
    const referral = await c.env.BOG_MENU_DB.prepare(
    ...
    const burgerRows = await c.env.BOG_MENU_DB.prepare(
    ...
    const referralTicketsRow = await c.env.BOG_MENU_DB.prepare(
    ...
    return json(200, {
      ...
      data: {
        ...
        shareUrl: `${originFromRequest(c.req.raw)}/?ref=${encodeURIComponent(referralCode)}`
      }
    });
  } catch {
