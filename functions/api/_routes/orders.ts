...
type SideQuestExtra = { sku?: string; name: string; price?: number; itemKind?: 'garnish' | 'drink' };
import {
  errorResponse,
  fetchOrderBundle,
  generateFolio,
...
const normalizeStringArray = (value: unknown, limit = 20) =>
  Array.isArray(value) ? value.map(normalizeString).filter(Boolean).slice(0, limit) : [];

const normalizeExtras = (value: unknown): ItemCustomization['extras'] =>
  Array.isArray(value)
    ? value
        .map((entry) => {
...
const normalizeSideQuestExtras = (value: unknown): ItemCustomization['sideQuestExtras'] =>
  Array.isArray(value)
    ? value
        .map((entry) => {
          if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return null;
...
const normalizeComboBurgers = (value: unknown): ItemCustomization['comboBurgers'] =>
  Array.isArray(value)
    ? value
        .map((entry) => {
          if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return null;
...
const validatePayload = (body: Record<string, unknown>, request: Request, towerConfigs: ActiveTowerConfig[] = []): NormalizedPayload | Response => {
  ...
  for (const rawItem of body.items) {
    ...
    if (
      extras.some((extra) => !extra.sku) ||
      (garnish && !garnish.sku) ||
      (includedDrink && !includedDrink.sku) ||
      sideQuestExtras.some((extra) => !extra.sku) ||
      comboBurgerExtras.some((extra) => !extra.sku)
    ) {
  ...
  normalizedItems.unshift(
    ...[...legacyQtyBySku.entries()].map(([sku, qty]) => ({
      sku,
      qty,
      removedIngredients: [],
      extras: [],
...
const orderHasReferralEligibleItem = (items: NormalizedPayload['items']) =>
  items.some((item) => (item.itemKind === 'burger' || item.itemKind === 'combo') && Math.max(0, Number(item.qty) || 0) > 0);
...
const safeLogOrderEvent = async (db: D1Database, params: { orderId: string; type: string; detail: Record<string, unknown>; now: string }) => {
  try {
    await db
      ...
      .bind(generateId('evt'), params.orderId, params.type, JSON.stringify(params.detail), params.now)
      .run();
  } catch {
...
const applyReferralCode = async (
  db: D1Database,
  params: { referralCode: string | null; orderId: string; customerPhone: string; customerName: string; eligibleForReferral: boolean; now: string }
) => {
  if (!params.referralCode) return undefined;
  if (!params.eligibleForReferral) {
    await safeLogOrderEvent(db, {
      orderId: params.orderId,
      type: 'RAFFLE_REFERRAL_SKIPPED',
      detail: { referralCode: params.referralCode, referralAccepted: false, reason: 'no_paid_burger_or_combo' },
      now: params.now
    });
    return false;
  }
  try {
    ...
    const codeRow = await db
      ...
      .bind(campaign.id, params.referralCode)
    ...
    await db
      ...
      .bind(
        ...
        params.customerName,
        Number(campaign.ticket_per_referral) || 2,
        params.now,
        params.now
      )
      .run();
    await db
      ...
      .bind(generateId('evt'), params.orderId, JSON.stringify({ referralCode: params.referralCode, referralAccepted: true }), params.now)
      .run();
    await safeLogOrderEvent(db, {
      orderId: params.orderId,
    ...
    try {
      await db
        ...
        .bind(generateId('evt'), params.orderId, JSON.stringify({ referralCode: params.referralCode, referralAccepted: false }), params.now)
        .run();
    } catch {
...
const ensureCustomerReferralCode = async (
  db: D1Database,
  params: { campaign: RaffleCampaignRow; ownerName: string; ownerPhone: string; orderId: string; now: string }
) => {
  const ownerPhone = normalizePhone(params.ownerPhone);
  if (!ownerPhone) return null;
  const existingOwner = await db
    .prepare('SELECT * FROM raffle_referral_codes_v2 WHERE campaign_id = ? AND owner_phone = ? LIMIT 1')
    .bind(params.campaign.id, ownerPhone)
    .first<ReferralCodeRow>();
  ...
  for (let attempt = 0; attempt < 72; attempt += 1) {
    ...
    const existingCode = await db
      .prepare('SELECT id FROM raffle_referral_codes_v2 WHERE campaign_id = ? AND code = ? LIMIT 1')
      .bind(params.campaign.id, code)
      .first<{ id: string }>();
    ...
    try {
      await db
        ...
        .bind(generateId('refcode'), params.campaign.id, ownerPhone, params.ownerName, code, params.now, params.now)
        .run();
      return code;
    } catch (error) {
      const ownerAfterCollision = await db
        .prepare('SELECT * FROM raffle_referral_codes_v2 WHERE campaign_id = ? AND owner_phone = ? LIMIT 1')
        .bind(params.campaign.id, ownerPhone)
        .first<ReferralCodeRow>();
...
const buildRaffleSuccessData = async (
  db: D1Database,
  params: { order: OrderV2; orderId: string; ownerName: string; ownerPhone: string; now: string }
) => {
  ...
  try {
    ...
    await safeLogOrderEvent(db, {
      orderId: params.orderId,
      type: 'RAFFLE_SUCCESS_DATA_SKIPPED',
      detail: { reason: 'active_campaign_lookup_failed' },
      now: params.now
    });
    return {};
  }
  ...
  try {
    ...
    await safeLogOrderEvent(db, {
      orderId: params.orderId,
      type: 'RAFFLE_TICKETS_SKIPPED',
      detail: { reason: 'earned_tickets_calculation_failed' },
      now: params.now
    });
  }
  try {
    const code = await ensureCustomerReferralCode(db, {
      campaign,
    ...
    await safeLogOrderEvent(db, {
      orderId: params.orderId,
      type: 'RAFFLE_CUSTOMER_CODE_SKIPPED',
      detail: { reason: 'customer_referral_code_generation_failed' },
      now: params.now
    });
  }
  return data;
...
ordersRouter.post('/', async (c) => {
  ...
  if (c.env.ORDERS_V2_WRITE_ENABLED !== 'true') return errorResponse(403, 'ORDERING_DISABLED', 'Órdenes V2 deshabilitadas temporalmente.');

  const body = await parseJsonObject(c.req.raw);
  if (!body) return errorResponse(400, 'INVALID_JSON', 'JSON inválido.');

  const towerConfigs = await loadActiveTowerConfigs(c.env.BOG_MENU_DB);
  const parsed = validatePayload(body, c.req.raw, towerConfigs);
  if (parsed instanceof Response) return parsed;
  const orderSource = getOrderSourceForEnvironment(parsed.environment);
  const isPreviewOrder = parsed.environment === 'preview';

  try {
    const existingRow = await c.env.BOG_MENU_DB.prepare('SELECT id FROM orders_v2 WHERE idempotency_key = ? AND source = ? LIMIT 1')
      .bind(parsed.idempotencyKey, orderSource)
      .first<{ id: string }>();
    if (existingRow?.id) {
      const existingOrder = await fetchOrderBundle(c.env.BOG_MENU_DB, existingRow.id);
      if (!existingOrder) return errorResponse(500, 'INTERNAL_ERROR', 'No se pudo recuperar la orden idempotente.');
      const raffleData = isPreviewOrder
        ? {}
        : await buildRaffleSuccessData(c.env.BOG_MENU_DB, {
            order: existingOrder,
            orderId: existingOrder.id,
            ownerName: existingOrder.customerName || parsed.customerName,
            ownerPhone: existingOrder.customerPhone || parsed.customerPhone,
            now: new Date().toISOString()
          });
      return json(200, {
        ok: true,
        data: { order: buildOrderSummary(existingOrder, parsed.idempotencyKey), idempotent: true, ...raffleData }
      });
    }
    ...
    const catalogRows = await loadCatalogRows(c.env.BOG_MENU_DB, skus);
    ...
    for (const item of parsed.items) {
      ...
      for (const extra of item.sideQuestExtras) {
        const catalogExtra = extra.sku ? catalogBySku.get(extra.sku) : null;
        const categoryKind: SideQuestExtra['itemKind'] | null =
          catalogExtra?.category_key === 'guarniciones' ? 'garnish' : catalogExtra && isDrinkCategory(catalogExtra.category_key) ? 'drink' : null;
    ...
    if (!isPreviewOrder && stockBySku.size > 0) {
      ...
      const stockStatements = stockEntries.map(([sku, qty]) =>
      ...
      const stockResult = await c.env.BOG_MENU_DB.batch(stockStatements);
      ...
      if (failedStock) {
        ...
        if (reservedEntries.length > 0) {
          await c.env.BOG_MENU_DB.batch(
            reservedEntries.map(([sku, qty]) =>
              c.env.BOG_MENU_DB!.prepare(
                `UPDATE menu_items
    ...
    const orderStatements: D1PreparedStatement[] = [
      c.env.BOG_MENU_DB.prepare(
        ...
        JSON.stringify(parsed.delivery),
        parsed.orderMode,
      ...
      c.env.BOG_MENU_DB.prepare(
        ...
        eventId,
        orderId,
        JSON.stringify({ source: orderSource, environment: parsed.environment, itemCount: orderItems.length, idempotencyKey: parsed.idempotencyKey }),
        orderSource,
        now
      )
    ];

    const orderResult = await c.env.BOG_MENU_DB.batch(orderStatements);
    if (!orderResult.every((entry) => entry.success)) {
      if (!isPreviewOrder && stockBySku.size > 0) {
        await c.env.BOG_MENU_DB.batch(
          [...stockBySku.entries()].map(([sku, qty]) =>
            c.env.BOG_MENU_DB!.prepare(
              `UPDATE menu_items
               SET stock_remaining = COALESCE(stock_remaining, 0) + ?,
                   sold_out_at = CASE WHEN COALESCE(stock_remaining, 0) + ? > 0 THEN NULL ELSE sold_out_at END,
    ...
    const referralAccepted = isPreviewOrder
      ? undefined
      : await applyReferralCode(c.env.BOG_MENU_DB, {
          referralCode: parsed.referralCode,
          orderId,
          customerPhone: parsed.customerPhone,
          customerName: parsed.customerName,
          eligibleForReferral: orderHasReferralEligibleItem(parsed.items),
          now
        });

    const createdOrder = await fetchOrderBundle(c.env.BOG_MENU_DB, orderId);
    ...
    const raffleData = isPreviewOrder
      ...
      : await buildRaffleSuccessData(c.env.BOG_MENU_DB, {
          order: createdOrder,
          orderId,
          ownerName: parsed.customerName,
          ownerPhone: parsed.customerPhone,
          now
        });
    return json(201, {
      ok: true,
      data: { order: buildOrderSummary(createdOrder, parsed.idempotencyKey), ...(referralAccepted === undefined ? {} : { referralAccepted }), ...raffleData }
    });
  } catch (err: any) {
