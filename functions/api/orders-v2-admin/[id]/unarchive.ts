...
export const onRequestPatch: PagesFunction<AdminEnv> = async ({ env, request, params }) => {
  ...
  try {
    ...
    const result = await env.BOG_MENU_DB.batch([
      env.BOG_MENU_DB.prepare('UPDATE orders_v2 SET archived_at = NULL, updated_at = ? WHERE id = ? AND archived_at IS NOT NULL').bind(now, id),
    ...
    if (!result.every((entry) => entry.success)) return errorResponse(500, 'INTERNAL_ERROR', 'No se pudo restaurar la orden desde el basurero.');
